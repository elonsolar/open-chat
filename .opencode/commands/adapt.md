...
指导方案：
    ...
    一、 适配流程：
        ...
        3. 修改目标平台模型 和打开模型思考模式,根据你的知识配置将模型能力配置最强
        4. 实现输入框发送消息功能：
         ...
         5. 实现消息监听和获取功能:
            1）分析dom:
                尝试发送多条信息，分析页面dom显示，查看消息格式， 消息一定有特殊的格式
                同时也要分析消息内部思考的内容，其通常也有特殊的格式

            2) 代码实现：
                通过MutationObserver 监测dom, 当dom 变化时,查找最后一条消息,最后一条消息你可以通过之前的消息标签，
                去除思考内容标签后，如果正文有约定好的结束标签，然后就结束监听，发送消息给后台
                在发送之前需要对消息格式做出一定个转换：
                    - 识别块级元素（P, DIV, H1-H6, PRE, TABLE 等）→ 加换行
                    - 识别标题 → 加 # 前缀
                    - 识别表格 → 转 markdown 表格语法
                    - 识别代码块 → 提取语言和代码内容，用  `` ` 包裹
                    - 识别行内元素 → 拼接文本
